...
y0         = [0.0]
xSpread    = [10.0]      #Fire shape factors
ySpread    = [0.3]
tIgnition  = [0.0]      #Fire's time of ignition
amplitude  = [0.5,0.5]  #[(max(0.1,10*dx))*sqrt(xScale*yScale),(max(0.1,10*dx))*sqrt(xScale*yScale)]      #Fire's initial spread (radius if circle)
...
res = GalacticOptim.solve(prob, GalacticOptim.ADAM(0.085), cb = cb, maxiters=maxIters) #allow_f_increase = false,
initθ = res.minimizer
...
res = GalacticOptim.solve(prob2, GalacticOptim.ADAM(0.001), cb = cb, maxiters=1800)
b_1 = time_ns()
...
gif(result, "level_set_stage_1_surface_1.gif", fps = FPS)

if true #maxlim > 0 && minlim < 0
    print("GOOD")
    ...
    gif(result_level, "level_set_stage_1_contour_1.gif", fps = FPS)
else
...
if printBCSComp
    ...
    png(bcsComparisonPlots, "bcs_comparison_stage_1.png")
    bcsComparisonPlots
end

##https://inciweb.nwcg.gov/incident/6758
##
