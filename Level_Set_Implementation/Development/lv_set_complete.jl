...
shapes     = ["zeroIsVertex","zeroIsCenter"]
x0         = [0.0]      #Fire ingnition coordinates
y0         = [0.0]
xSemiAxis  = [1.0]      #Fire shape factors
ySemiAxis  = [1.0]
tIgnition  = [0.0]      #Fire's time of ignition
amplitudes = [2.0]      #Fire's initial spread (radius if circle)

domainShape = shapes[2]

if domainShape == shapes[1]
    domains = [t ∈ IntervalDomain(0.0,tmax),
           x ∈ IntervalDomain(0.0,xwidth),
           y ∈ IntervalDomain(0.0,ywidth)]
elseif domainShape == shapes[2]
    domains = [t ∈ IntervalDomain(0.0,tmax),
