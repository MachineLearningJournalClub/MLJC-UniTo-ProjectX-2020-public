...
#topography
#z = p0 + p1*x + p2*x*x + p3*x*x*x + p4*x*x*x*x + p5*x*x*x*x*x + p6*y + p7*y*y + p8*y*y*y + p9*y*y*y*y +
#      p10*y*y*y*y*y + p11*x*y + p12*x*x*y + p13*x*y*y + p14*x*x*y*y
...
u_predict = [reshape([first(phi([t,x,y],pars)) for x in xs for y in ys], (length(xs),length(ys))) for t in ts]
...
gif(result, "isom_creek_surface.gif", fps = FPS)
...
gif(result_level, "isom_creek_contour.gif", fps = FPS)
=#
