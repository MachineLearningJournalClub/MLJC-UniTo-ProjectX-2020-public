...
# Short description of this file: Poisson's equation 2D
...
chain = Chain(Dense(dim,16,Flux.tanhshrink),Dense(16,16,Flux.tanhshrink),Dense(16,1))
...
savefig("c_real.pdf")
plot(p2)
savefig("c_predict.pdf")
plot(p3)
savefig("c_diff.pdf")

p11 = Plots.plot(xs, ys, u_real, linetype=:surface,title = "analytic");
p22 = Plots.plot(xs, ys, u_predict, linetype=:surface,title = "predict");
p33 = Plots.plot(xs, ys, diff_u,linetype=:surface,title = "error");
plot(p11)
savefig("s_real.pdf")
plot(p22)
savefig("s_predict.pdf")
plot(p33)
savefig("s_diff.pdf")
